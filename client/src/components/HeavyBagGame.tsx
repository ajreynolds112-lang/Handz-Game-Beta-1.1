...
export default function HeavyBagGame({ fighter, onComplete, onQuit }: HeavyBagGameProps) {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (stateRef.current.paused) {
      if (finishEarlyConfirmRef.current) {
        if (e.key === "Enter") {
          e.preventDefault();
          onComplete(Math.floor(combosRef.current * XP_PER_COMBO), combosRef.current);
        }
        return;
      }
      ...
      if (e.key === "Enter") {
        ...
        setPauseIndex(prev => {
          ...
          else if (prev === 2 && fighter.wins >= 12) {
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    menuItems.forEach((_, i) => {
      ...
      if (cx > W / 2 - 80 && cx < W / 2 + 80 && cy > iy - 15 && cy < iy + 10) {
        ...
        else if (i === 2 && fighter.wins >= 12) {
          setFinishEarlyConfirm(true);
          finishEarlyConfirmRef.current = true;
        }
      }
    });
  }, [paused, finished, onQuit, fighter, finishEarlyConfirm]);
  ...
  useEffect(() => {
    ...
    if (paused && !finished) {
      ...
      if (finishEarlyConfirm) {
        ...
        ctx.fillStyle = "#aaaacc";
        ctx.font = "13px monospace";
        ctx.fillText("[ENTER] Confirm   [ESC] Cancel", W / 2, H / 2 + 32);
      } else {
        const menuItems = ["Resume", "Quit", "Finish Early"];
        menuItems.forEach((item, i) => {
          const y = H / 2 - 10 + i * 35;
          const isLocked = i === 2 && fighter.wins < 12;
          const isSelected = i === pauseIndex;
          ctx.fillStyle = isLocked ? "#555555" : isSelected ? "#ffcc00" : "#888888";
          ctx.font = (isSelected && !isLocked) ? "bold 18px monospace" : "16px monospace";
          const label = isLocked ? `Finish Early [${fighter.wins}/12 wins]` : item;
          ctx.fillText(`${(isSelected && !isLocked) ? "> " : "  "}${label}`, W / 2, y);
        });
      }
    }
  ...
  }, [countdown, timeLeft, combos, currentCombo, comboIndex, missFlash, hitFlash, paused, finished, pauseIndex, bagSwing, fighter, skinColor, gc, currentPunch, bobPhase, finishEarlyConfirm]);
