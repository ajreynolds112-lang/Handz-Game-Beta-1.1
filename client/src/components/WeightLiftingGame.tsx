...
export default function WeightLiftingGame({ fighter, onComplete, onQuit }: WeightLiftingGameProps) {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    if (stateRef.current.paused) {
      if (finishEarlyConfirmRef.current) {
        if (e.key === "Enter") {
          e.preventDefault();
          onComplete(Math.floor(repsRef.current * XP_PER_REP), repsRef.current);
        }
        return;
      }
  ...
  }, [countdown, timeLeft, presses, reps, paused, finished, pressProgress, currentNeeded, pauseIndex, fighter, skinColor, gc, bonusFloats, finishEarlyConfirm]);
