...
export function ensureEndgamePool(state: CareerRosterState, playerLevel: number, seed: number): CareerRosterState {
  if (playerLevel < 100) return state;
  const activeIds = state.roster.filter(f => f.active && !f.retired && f.rank > 0).map(f => f.id);
  const activeIdSet = new Set(activeIds);
  let shown = (state.endgameShownIds ?? []).filter(id => activeIdSet.has(id));
  if (shown.length >= Math.min(20, activeIds.length)) {
    // Pool already established; leave it (and its jumped levels) untouched.
    return state;
  }
  const rng = seededRandom(seed);
  const pool = shuffleInPlace(activeIds.filter(id => !shown.includes(id)), rng);
  shown = [...shown, ...pool].slice(0, 20);
  const lo = Math.max(1, playerLevel - 5);
  const hi = playerLevel + 30;
  const shownSet = new Set(shown);
  const newRoster = state.roster.map(f => {
    if (!shownSet.has(f.id)) return f;
    let level = lo + Math.floor(rng() * (hi - lo + 1));
    if (f.id === 204) level = Math.max(100, level);
    return { ...f, level };
  });
  return { ...state, roster: newRoster, endgameShownIds: shown };
}

export function redistributeRosterLevels(roster: RosterFighterState[], maxLevelGainPerWeek: number = 1, playerLevel?: number, endgameShownIds?: number[]): RosterFighterState[] {
  const result = roster.map(f => ({ ...f }));
  const rng = seededRandom(result.length * 31 + 7);
  const endgame = playerLevel != null && playerLevel >= 100;
  const ceiling = endgame ? playerLevel + 30 : 100;
  const endgameLow = endgame && playerLevel != null ? Math.max(1, playerLevel - 5) : 1;
  const shownSet = new Set(endgameShownIds ?? []);
  ...
  for (const f of active) {
    if (endgame && shownSet.has(f.id)) {
      // Preserve the endgame shown pool's jumped level; only clamp into [player-5, player+30].
      f.level = Math.max(endgameLow, Math.min(ceiling, f.level));
      continue;
    }
  ...
  if (champ) champ.level = endgame ? Math.max(champ.level, 100) : 100;
  return result;
}
...
export function getOpponentCandidates(
  ...
  if (playerLevel !== undefined && playerLevel >= 100) {
    // Level 100 endgame: only the curated weekly pool of 20 fighters is shown, all available.
    const shownSet = new Set(endgameShownIds ?? []);
    let shown = active.filter(f => shownSet.has(f.id));
    if (shown.length < 20) {
      // Pool missing or stale: top up to 20 with the next fighters by rank.
      const extra = [...active]
    ...
    return shown
      .map(f => ({ ...f, unavailableThisWeek: false }))
      .sort((a, b) => a.rank - b.rank)
      .slice(0, 20);
  }

  if (isChampBeaten(roster)) {
    // Champion defeated: open roster unlocked with no level cap, so any active fighter can be challenged.
    return active
      .map(f => ({ ...f, unavailableThisWeek: false }))
      .sort((a, b) => a.rank - b.rank);
  }
...
export function simulateWeek(state: CareerRosterState, weekSeed: number, careerDifficulty?: string, playerLevel?: number): CareerRosterState {
  ...
  const endgame = playerLevel != null && playerLevel >= 100;
  if (!endgame && state.selectedOpponentId != null && careerDifficulty && playerLevel != null) {
    const opp = newRoster.find(f => f.id === state.selectedOpponentId);
    if (opp) {
      const levelUpChance = Math.min(0.95, careerDifficulty === "champion" ? 0.60 : careerDifficulty === "elite" ? 0.50 : careerDifficulty === "contender" ? 0.40 : 0.30);
      const doubleUpChance = Math.min(0.60, careerDifficulty === "champion" ? 0.20 : careerDifficulty === "elite" ? 0.15 : careerDifficulty === "contender" ? 0.10 : 0.05);
      ...
      const normalCap = playerLevel + (prepWeeks > 4 ? longCap : shortCap);
      const earlyFightCap = (state.careerFightsCompleted ?? 0) < 5 ? playerLevel + 3 : Infinity;
      const effectiveCap = Math.min(normalCap, earlyFightCap);
      const ceiling = 100;
      if (rng() < levelUpChance && opp.level < effectiveCap && opp.level < ceiling) {
        opp.level++;
        if (rng() < doubleUpChance && opp.level < effectiveCap && opp.level < ceiling) {
          opp.level++;
        }
      }
    }
  ...
  if (wolf && wolf.active) wolf.level = endgame ? Math.max(wolf.level, 100) : 100;
  ...
  if (endgame && playerLevel != null) {
    ...
    if (shown.length < 20) {
      // Initialize (or refill) the weekly pool up to 20 random active fighters.
      const pool = shuffleInPlace(activeIds.filter(id => !shown.includes(id)), rng);
      shown = [...shown, ...pool].slice(0, 20);
    } else {
      // Rotate 5-10 fighters out for fresh random active fighters.
      const rotateCount = 5 + Math.floor(rng() * 6);
      const shownShuffled = shuffleInPlace([...shown], rng);
      const keep = shownShuffled.slice(rotateCount);
      const incoming = shuffleInPlace(activeIds.filter(id => !shown.includes(id)), rng).slice(0, rotateCount);
      shown = [...keep, ...incoming];
      if (shown.length < 20) {
        // Not enough fresh fighters to fill the pool; top back up from the rotated-out set.
        const fill = shownShuffled.slice(0, rotateCount).filter(id => !shown.includes(id));
        shown = [...shown, ...fill].slice(0, 20);
      }
    }

    // Jump the shown pool into the player's challenge band [player-5, player+30].
    const lo = Math.max(1, playerLevel - 5);
    const hi = playerLevel + 30;
    const shownSet = new Set(shown);
    for (const f of newRoster) {
      if (shownSet.has(f.id)) {
        let level = lo + Math.floor(rng() * (hi - lo + 1));
        if (f.id === 204) level = Math.max(100, level);
        f.level = level;
      }
    }
    newEndgameShownIds = shown;
  }
