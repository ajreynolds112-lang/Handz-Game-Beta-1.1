...
function drawRingBackRopes(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  edges.forEach(([p1, p2]) => {
    ...
    ropeHeights.forEach((h, i) => {
      ...
      const ropeDark = shadeColor(ropeCol === "#ffffff" ? "#888888" : ropeCol, -60);
      const ropeLight = shadeColor(ropeCol, 40);
  ...
  corners.forEach(([cx, cz], idx) => {
    ...
    const postDark = shadeColor(postColor, -50);
    const postLight = shadeColor(postColor, 40);
    ...
    postGrad.addColorStop(0.65, postColor);
...
function drawFighter(ctx: CanvasRenderingContext2D, fighter: FighterState, opponent: FighterState, state: GameState, screenPt: ScreenPt, oppPt: ScreenPt): void {
  ...
  if (isChargeFlashing) {
    ...
    torsoGrad.addColorStop(0.45, skinBase);
  ...
  if (!critFlash && !isChargeFlashing) {
    ctx.strokeStyle = shadeColor(c.skin, 40);
    ctx.lineWidth = 1;
    ctx.globalAlpha = 0.35;
    torsoPath();
    ctx.stroke();
  ...
  if (critFlash) {
    ...
    const trunkGrad = ctx.createLinearGradient(trunkTopX - shoulderW, trunkTop, trunkTopX + shoulderW, trunkTop);
    const trunkDark = shadeColor(c.trunks, -40);
    const trunkLight = shadeColor(c.trunks, 30);
    trunkGrad.addColorStop(0, trunkDark);
    trunkGrad.addColorStop(0.4, c.trunks);
  ...
  const headCX = bodyX + headShift + headSlideOX + weaveHeadOX;
  const headCY = headY + forwardOffY + headSlideOY + headVibY + weaveHeadOY;
  ...
  if (chargeFlash) {
    ...
    headGrad.addColorStop(0.5, c.skin);
...
function drawLegs(
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    if (critFlash) {
      ...
      // 3D leg: shadow underlay + lit surface + highlight rim
      const skinDark = shadeColor(c.skin, -45);
      const skinLight = shadeColor(c.skin, 18);
      ctx.strokeStyle = skinDark;
      ctx.lineWidth = lineW + 2;
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(hipX + 1.5, hipY + 1);
      ctx.lineTo(kneeX + 1.5, kneeY + 1);
      ctx.stroke();
      ...
      ctx.globalAlpha = 0.35;
      ctx.beginPath();
      ctx.moveTo(hipX - 1, hipY - 0.5);
      ctx.lineTo(kneeX - 1, kneeY - 0.5);
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
    ...
    if (critFlash) {
      ...
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(kneeX + 1.5, kneeY + 1);
      ctx.lineTo(footX + 1.5, footY + 1);
      ctx.stroke();
      ctx.strokeStyle = c.skin;
      ctx.lineWidth = lineW;
      ctx.beginPath();
      ...
      ctx.globalAlpha = 0.3;
      ctx.beginPath();
      ctx.moveTo(kneeX - 1, kneeY - 0.5);
      ctx.lineTo(footX - 1, footY - 0.5);
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
    ...
    const shoeH = 3.5 * FIGHTER_SCALE * legScale;
    const shoeAngle = Math.atan2(0, punchDirX);
    if (critFlash) {
      ctx.fillStyle = "#cc1111";
    } else {
      const shoeGrad = ctx.createLinearGradient(
        footX - shoeW * 0.3, footY - shoeH,
        footX - shoeW * 0.3, footY + shoeH
      );
      shoeGrad.addColorStop(0, shadeColor(c.shoes, 20));
      shoeGrad.addColorStop(0.5, c.shoes);
      shoeGrad.addColorStop(1, shadeColor(c.shoes, -40));
    ...
    ctx.fill();
    if (!critFlash) {
      ctx.strokeStyle = shadeColor(c.shoes, -50);
      ctx.lineWidth = 0.5;
      ctx.globalAlpha = 0.5;
      ctx.stroke();
      ctx.globalAlpha = 1;
...
function drawArms(
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    // Upper arm: thicker at shoulder, thinner at elbow for tapered 3D look
    const armThick = 4 * FIGHTER_SCALE;
    if (critFlash) {
      ctx.strokeStyle = "#ff2222";
      ...
      // 3D arm shading: paint two strokes (shadow + lit)
      const skinDark = shadeColor(c.skin, -40);
      ...
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(shoulderX + 1.5, sY + 1);
      ctx.lineTo(elbowX + 1.5, elbowY + 1);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(elbowX + 1.5, elbowY + 1);
      ctx.lineTo(gloveX + 1.5, gloveY + 1);
      ctx.stroke();
      // Lit layer
      ctx.strokeStyle = c.skin;
      ctx.lineWidth = armThick;
      ...
      ctx.globalAlpha = 0.4;
      ctx.beginPath();
      ctx.moveTo(shoulderX - 1, sY - 0.5);
      ctx.lineTo(elbowX - 1, elbowY - 0.5);
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
    ...
    ctx.beginPath();
    ctx.arc(elbowX, elbowY, 2.8 * FIGHTER_SCALE, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (critFlash || blockFlash) {
      ctx.fillStyle = rawGloveColor;
      ctx.beginPath();
      ctx.arc(gloveX, gloveY, GLOVE_R, 0, Math.PI * 2);
      ctx.fill();
    } else {
      // Radial gradient for 3D sphere glove
      const gloveGrad = ctx.createRadialGradient(
        gloveX - GLOVE_R * 0.3, gloveY - GLOVE_R * 0.3, GLOVE_R * 0.05,
        gloveX, gloveY, GLOVE_R * 1.1
      );
      gloveGrad.addColorStop(0, shadeColor(rawGloveColor, 50));
      gloveGrad.addColorStop(0.5, rawGloveColor);
      gloveGrad.addColorStop(1, shadeColor(rawGloveColor, -55));
      ctx.fillStyle = gloveGrad;
      ctx.beginPath();
      ctx.arc(gloveX, gloveY, GLOVE_R, 0, Math.PI * 2);
      ctx.fill();
      // Glove rim shadow
      ctx.strokeStyle = shadeColor(rawGloveColor, -70);
      ctx.lineWidth = 0.7;
...
function drawHUD(ctx: CanvasRenderingContext2D, state: GameState): void {
  ...
  // Enemy rhythm meter (right side)
  const eRhythmStartX = CANVAS_W - padding - rhythmW;
  const eRhythmLevel = state.enemy.rhythmLevel;
  const eRhythmProgress = state.enemy.rhythmProgress;

  ctx.fillStyle = "rgba(255,255,255,0.1)";
  ctx.fillRect(eRhythmStartX, rhythmY, rhythmW, 6);

  if (eRhythmLevel > 0) {
    ctx.fillStyle = "rgba(100, 180, 255, 0.2)";
    ctx.fillRect(eRhythmStartX, rhythmY, rhythmW * 0.4, 6);
    ctx.fillStyle = "rgba(100, 255, 100, 0.2)";
    ctx.fillRect(eRhythmStartX + rhythmW * 0.4, rhythmY, rhythmW * 0.2, 6);
    ctx.fillStyle = "rgba(255, 180, 100, 0.2)";
    ctx.fillRect(eRhythmStartX + rhythmW * 0.6, rhythmY, rhythmW * 0.4, 6);

    const eMarkerX = eRhythmStartX + eRhythmProgress * rhythmW;
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(eMarkerX - 1, rhythmY - 1, 2, 8);

    ctx.font = "7px 'Oxanium', sans-serif";
    ctx.textAlign = "right";
    ctx.fillStyle = "rgba(255,255,255,0.5)";
    ctx.fillText(`R${eRhythmLevel}`, eRhythmStartX - 3, rhythmY + 5);
  } else {
    ctx.font = "7px 'Oxanium', sans-serif";
    ctx.textAlign = "right";
    ctx.fillStyle = "rgba(255,255,255,0.3)";
    ctx.fillText("R0", eRhythmStartX - 3, rhythmY + 5);
  }
...
function createPreviewFighterState(colors: FighterColors, bobPhase: number): FighterState {
  return {
    ...
    weaveCounterTimer: 0,
    weaveFreezeTimer: 0,
    weaveStartedAfterPunchThrown: false,
  };
}
