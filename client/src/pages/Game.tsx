...
const MAX_STAT_POINTS = 780;

function totalSkillPts(sp: unknown, avail = 0): number {
...
const FIGHT_MILESTONES: Array<{
  ...
  { type: "wins", threshold: 6, title: "Training Upgrade!", description: "Weight Lifting now builds Focus. Heavy Bag training grants 1.2× XP.", icon: "💪" },
  { type: "bouts", threshold: 7, title: "Fight Camp Stamina!", description: "Weight Lifting now builds Stamina while preparing for a scheduled fight.", icon: "🏋️" },
  { type: "wins", threshold: 15, title: "Sparring Mastery!", description: "All stats are now available in every sparring session.", icon: "🥊" },
  { type: "bouts", threshold: 15, title: "Iron Conditioning!", description: "Weight Lifting now always builds Stamina alongside Power and Defense.", icon: "🏆" },
...
export default function Game() {
  ...
  const [pendingMilestones, setPendingMilestones] = useState<Array<{ title: string; description: string; icon: string }>>([]);
  ...
  const handleStateChange = useCallback((newState: GameState) => {
    ...
    if (newState.phase === "fightEnd" && uiMode === "fighting") {
      ...
      if (isSparring && activeFighter) {
        const SPARRING_XP_BOOST = 1.3;
        const sparringWon = newState.fightWinner === "player";
        const sparLevelTrainingMult = activeFighter.level >= 100 ? 1.0 : activeFighter.level >= 20 ? 1.2 : 1.0;
        ...
        const xpGained = Math.max(1, Math.floor(sparringBase * sparringDiffMult * winMult * sparBoutScale * sparLevelTrainingMult * 0.32));
        ...
        const sparMaxByDiff = sparIdleWeeks === 1 ? Math.ceil(sparMaxByDiffBase / 2) : sparMaxByDiffBase;
        sparringAllocPoints = Math.min(sparringAllocPoints, sparMaxByDiff);
        sparringAllocPoints = Math.min(
          sparringAllocPoints,
          Math.max(0, MAX_STAT_POINTS - totalSkillPts(activeFighter.skillPoints, activeFighter.availableStatPoints || 0))
        );
        if (sparIdleWeeks >= 2) {
        ...
        const sparringBonusXp = Math.floor(
          ...
          (newState.player.feintBaits * 20)) * sparFightScaleMult * 0.7 * 0.32
        );

        const sparRs = activeFighter.careerRosterState as CareerRosterState | null;
        const sparPrepMult = sparRs?.prepWeeksRemaining === 2 ? 2 : 1;
        const sparTb = (activeFighter.trainingBonuses || DEFAULT_TRAINING_BONUSES) as TrainingBonuses;
        const sparPostFightBoost = (sparTb.postFightXpBoost && (sparTb.sparring || 0) < 7) ? Math.pow(1.3, sparTb.postFightXpBoost) : 1.0;
        ...
        const sparEndgameMult = activeFighter.level >= 100 ? 0.9 : 1;
        const sparRosterForXp = activeFighter.careerRosterState as CareerRosterState | null;
        const sparPreChampXpMult = sparRosterForXp && isChampBeaten(sparRosterForXp.roster) ? 0.9 : 0.5;
        const sparChampBeaten = sparRosterForXp ? isChampBeaten(sparRosterForXp.roster) : false;
        const sparXpRaw = Math.floor((xpGained + sparringBonusXp) * SPARRING_XP_BOOST * sparPrepMult * sparPostFightBoost * sparEndgameMult * sparPreChampXpMult * CAREER_XP_MULT);
        const sparXpFinal = sparIdleWeeks >= 2 ? Math.max(1, Math.ceil(sparXpRaw * 0.5)) : sparIdleWeeks === 1 ? Math.max(1, Math.ceil(sparXpRaw * 0.85)) : sparXpRaw;
        let newXp = midFightXp + sparXpFinal;
        let newLevel = midFightLevel;
        while (newXp >= xpToNextLevel(newLevel) && (sparChampBeaten || newLevel < 100)) {
          newXp -= xpToNextLevel(newLevel);
          newLevel++;
        }
        if (!sparChampBeaten && newLevel >= 100) {
          newLevel = 100;
          newXp = Math.min(newXp, xpToNextLevel(100) - 1);
        }

        const oldStats = (activeFighter.careerStats || DEFAULT_CAREER_STATS) as CareerStats;
        const newCareerStats: CareerStats = {
          ...oldStats,
          lifetimeXp: oldStats.lifetimeXp + (sparIdleWeeks >= 2 ? Math.max(1, Math.ceil(Math.floor((xpGained + sparringBonusXp) * SPARRING_XP_BOOST * sparPreChampXpMult * CAREER_XP_MULT) * 0.5)) : sparIdleWeeks === 1 ? Math.max(1, Math.ceil(Math.floor((xpGained + sparringBonusXp) * SPARRING_XP_BOOST * sparPreChampXpMult * CAREER_XP_MULT) * 0.85)) : Math.floor((xpGained + sparringBonusXp) * SPARRING_XP_BOOST * sparPreChampXpMult * CAREER_XP_MULT)),
        };
        ...
        if (updatedRosterState) {
          ...
          updatedRosterState = simulateWeek(updatedRosterState, weekSeed, activeFighter.careerDifficulty, newLevel);
        ...
        const sparCustomUnlock = sparTb.customStatUnlock as keyof SkillPoints | undefined;
        const sparBaseAllowed: (keyof SkillPoints)[] = activeFighter.level >= 50 ? ["speed", "defense", "stamina", "focus"] : ["speed", "defense", "stamina"];
        const sparTotalSessions = (newTB.weightLifting || 0) + (newTB.heavyBag || 0) + (newTB.sparring || 0);
        // After 12 total training sessions, sparring lets the player allocate across all 5 skills.
        const sparAllSkillsUnlocked = sparTotalSessions >= 12;
        ...
        const champWinBonus = isWin && newState.aiDifficulty === "champion" ? 1.5 : 1.0;
        const endgameXpMult = activeFighter.level >= 100 ? 0.3 : 1.0;
        const fightRosterForXp = activeFighter.careerRosterState as CareerRosterState | null;
        const fightPreChampXpMult = fightRosterForXp && isChampBeaten(fightRosterForXp.roster) ? 1.0 : 0.5;
        const totalXp = Math.floor((newState.xpGained * fightBonusMult + fightBonusXp) * accuracyMult * champWinBonus * endgameXpMult * fightPreChampXpMult * CAREER_XP_MULT * 2);
        const midFightLevel = newState.midFightLevelUps > 0 ? newState.playerLevel : activeFighter.level;
        const midFightXp = newState.midFightLevelUps > 0 ? newState.playerCurrentXp : activeFighter.xp;
        ...
        let newLevel = midFightLevel;
        while (newXp >= xpToNextLevel(newLevel) && (fightChampBeaten || newLevel < 100)) {
          newXp -= xpToNextLevel(newLevel);
          newLevel++;
        }
        if (!fightChampBeaten && newLevel >= 100) {
          newLevel = 100;
          newXp = Math.min(newXp, xpToNextLevel(100) - 1);
        }
        ...
        const newCareerStats: CareerStats = {
          ...
          lifetimeXp: oldStats.lifetimeXp + Math.floor(newState.xpGained * fightPreChampXpMult * CAREER_XP_MULT),
        };
        ...
        if (careerOpponentId === 204 && isWin && !wasChampBeaten) {
          allTriggered.push({
            ...
            description: "The level 100 cap has been removed — you can now grow beyond level 100. Free skin color selection and endgame opponents are now unlocked.",
            icon: "👑",
          });
        }
        ...
        if (allTriggered.length > 0) {
          setPendingMilestones(allTriggered);
        }
        ...
        if (updatedRosterState && newLevel >= 100) {
          // Reaching level 100 via an official fight: make sure the endgame pool exists and is level-jumped.
        ...
        saveFightResultMutation.mutate({
          ...
          xpGained: Math.floor(newState.xpGained * fightPreChampXpMult * CAREER_XP_MULT),
  ...
  const handleStartFight = (playerLevel: number, enemyLevel: number, name?: string, isQuick: boolean = false, enemyArch?: Archetype, careerAiDiff?: AIDifficulty, careerRoundLen?: number, eArm?: number, enemyName?: string, enemyColors?: FighterColors, overridePlayerColors?: FighterColors, overrideFighter?: Fighter, careerRounds?: number, careerSpeed?: TimerSpeed, enemyRosterId?: number, enemyRank?: number, rosterStats?: { power?: number; speed?: number; defense?: number; stamina?: number; focus?: number }) => {
    ...
    if (effectiveCareerFight || isSparring) {
      ...
      newState.careerXpMult = (rs && isChampBeaten(rs.roster) ? 1.0 : 0.5) * CAREER_XP_MULT;
      newState.canSurpassLevel100 = rs ? isChampBeaten(rs.roster) : false;
  ...
  const handleContinue = () => {
    ...
    if (wasSparring && deferredMilestoneRef.current) {
  ...
  const handleTrainingComplete = (xpGained: number, reps: number) => {
    ...
    const hbBonusXp = trainingType === "heavyBag" && (activeFighter.wins || 0) >= 6 ? 1.2 : 1;
    const levelTrainingMult = activeFighter.level >= 100 ? 1.0 : activeFighter.level >= 20 ? 1.2 : 1.0;
    ...
    const endgameTrainXpMult = endgameTrain ? 0.8 : 1;
    const trainPreChampXpMult = trainRs && isChampBeaten(trainRs.roster) ? 1.0 : 0.5;
    const repBonusMult = reps > 30 ? Math.pow(1.2, Math.floor((reps - 30) / 5)) : 1.0;
    const trainIdleWeeks = (trainRs?.selectedOpponentId != null && (trainRs?.prepWeeksRemaining ?? 0) > 0) ? 0 : (trainRs?.idleWeeks ?? 0);
    const scaledXpBase = Math.max(1, Math.floor(xpGained * 0.7 * (1 + activeFighter.level * 0.04) * fightMult * trainPrepMult * wlBonusXp * hbBonusXp * postFightBoost * levelTrainingMult * endgameTrainXpMult * trainPreChampXpMult * CAREER_XP_MULT * repBonusMult));
    const scaledXp = trainIdleWeeks >= 2
      ? Math.max(1, Math.ceil(scaledXpBase * 0.5))
      : trainIdleWeeks === 1
      ? Math.max(1, Math.ceil(scaledXpBase * 0.85))
      : scaledXpBase;
    ...
    while (newXp >= xpToNextLevel(newLevel) && (trainChampBeaten || newLevel < 100)) {
      newXp -= xpToNextLevel(newLevel);
      newLevel++;
    }
    if (!trainChampBeaten && newLevel >= 100) {
      newLevel = 100;
      newXp = Math.min(newXp, xpToNextLevel(100) - 1);
    }
    ...
    let allocPoints = trainingType === "weightLifting"
      ? Math.max(1, Math.floor(reps / (endgameTrain ? 5 : 3)))
      : Math.max(1, Math.floor(reps / (endgameTrain ? 5 : 3)));
    ...
    allocPoints = Math.ceil(allocPoints * 1.75);
    if (trainingType === "weightLifting") {
      allocPoints = Math.floor(allocPoints * 0.5);
    } else if (trainingType === "heavyBag") {
      allocPoints = Math.floor(allocPoints * 0.75);
    }
    allocPoints = Math.min(
      allocPoints,
      Math.max(0, MAX_STAT_POINTS - totalSkillPts(activeFighter.skillPoints, activeFighter.availableStatPoints || 0))
    );
    const trainHardCap = trainIdleWeeks === 1 ? Math.ceil(10 / 2) : 10;
    ...
    const trainCustomUnlock = tb.customStatUnlock as keyof SkillPoints | undefined;
    const baseWlStats: (keyof SkillPoints)[] = ["power", "defense"];
    if (wlFocusUnlocked) baseWlStats.push("focus");
    if (wlStaminaUnlocked) baseWlStats.push("stamina");
    const baseAllowedStats: (keyof SkillPoints)[] = trainingType === "weightLifting"
      ? baseWlStats
      : ["power", "speed", "focus"];
    const allowedStats: (keyof SkillPoints)[] = trainCustomUnlock && !baseAllowedStats.includes(trainCustomUnlock)
      ? [...baseAllowedStats, trainCustomUnlock]
      : baseAllowedStats;
    ...
    const trainLevelMilestones = FIGHT_MILESTONES
      .filter(m => m.type === "level" && activeFighter.level < m.threshold && newLevel >= m.threshold)
      .map(m => ({ title: m.title, description: m.description, icon: m.icon }));
    if (trainLevelMilestones.length > 0) {
      setPendingMilestones(trainLevelMilestones);
    }

    const totalSessions = (newTB.weightLifting || 0) + (newTB.heavyBag || 0) + (newTB.sparring || 0);
    const needsStatUnlock = totalSessions >= 10 && !tb.customStatUnlock;

    if (needsStatUnlock) {
      setStatUnlockPending({
        allocData: allocPoints > 0 ? {
          points: allocPoints,
          allowedStats,
          didSimulate,
          simulationNews: didSimulate && updatedRosterState ? updatedRosterState.newsItems : undefined,
    ...
    if (allocPoints <= 0) {
      // No stat points to allocate (e.g. unscheduled training after level 50) — skip the allocation screen.
  ...
  const handleStatUnlockPick = (stat: keyof SkillPoints) => {
    ...
    const updatedTB: TrainingBonuses = { ...tbToSave, customStatUnlock: stat as string };
    ...
    if (allocData) {
      ...
      setPendingTrainingAlloc({ ...allocData, allowedStats: allowedWithUnlock });
      setUiMode("trainingAllocate" as UIMode);
  ...
  const handleAllocateStats = (fighterId: string, skillPoints: SkillPoints, spent: number) => {
    const currentAvailable = (activeFighter?.id === fighterId)
      ? (activeFighter?.availableStatPoints || 0)
      : (localSaves.getFighter(fighterId)?.availableStatPoints || 0);
    const newAvailable = currentAvailable - spent;
    updateFighterMutation.mutate({
      id: fighterId,
      data: {
        skillPoints,
      ...
      <div className="w-full mx-auto">
        {(uiMode === "fighting" || uiMode === "fightEnd") && (
          <div className="relative">
            <GameCanvas state={gameState} onStateChange={handleStateChange} careerDynamicMusic={dynamicMusicEnabled} />
        ...
        {uiMode === "tutorial" && (
          <div className="flex flex-col items-center justify-center min-h-screen gap-6 p-6" data-testid="tutorial-screen">
            ...
            <div className="flex flex-col gap-3 w-full max-w-xs">
              <button
                onClick={() => startTutorialFight(tutorialStage)}
